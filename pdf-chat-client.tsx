...
export function PdfChatClient({ initialDocuments, initialUsage }: { initialDocuments: SavedDocument[]; initialUsage?: UsageStatus }) {
  ...
  async function handleDeleteDocument(document: SavedDocument) {
    const confirmed = window.confirm(`"${document.title}" wirklich löschen? Der Chatverlauf zu dieser PDF wird ebenfalls entfernt.`);
    ...
    try {
      ...
      setDocuments((currentDocuments) => {
        const nextDocuments = currentDocuments.filter((currentDocument) => currentDocument.id !== document.id);

        if (selectedDocument?.id === document.id) {
          const nextSelectedDocument = nextDocuments[0] ?? null;
          setSelectedDocument(nextSelectedDocument);
          setMessages([]);

          if (nextSelectedDocument) {
            void selectDocument(nextSelectedDocument);
          }
        }
          ...
          {usage?.plan !== "pro" ? (
            ...
            <div className="mt-5 space-y-3">
              ...
              <form action={createBillingPortalSession}>
                <button className="w-full rounded-xl border border-white/15 px-4 py-3 font-semibold text-white transition hover:bg-white/10">
            ...
            {documents.length === 0 ? (
              ...
              documents.map((document) => {
                ...
                return (
                  ...
                  >
                    <button type="button" onClick={() => selectDocument(document)} className="w-full text-left">
                    ...
                    <button
                      ...
                      className="mt-3 rounded-lg border border-red-300/20 px-3 py-2 text-xs font-medium text-red-200 transition hover:bg-red-400/10 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                      {deletingDocumentId === document.id ? "Wird gelöscht..." : "🗑 Löschen"}
                    </button>
                  </div>
                );
              })
